...
const Kitten: React.FC<{ kitten: KittenProps }> = ({ kitten }) => {
    ...
    return (
        <Link href={'/kitten/' + kitten.id}>
            <div className="grid items-center justify-center max-w-xs grid-cols-2 mx-auto text-center transition-colors shadow-lg card bg-base-100 hover:bg-secondary ">
                {kitten.image ? (
                    <img src={kitten.image} className="object-cover w-40 h-40" alt={kitten.name} />
                ) : (
                    <img
                        src="/cat.png"
                        className="object-cover w-40 h-40 p-6 bg-primary"
                        alt={kitten.name}
                    />
                )}
                <div>
